...
should be dropped in favor of a delivered voice reply.
"""
...
def test_suppress_off_never_suppresses_even_with_voice():
    ...
    assert ad._should_suppress_text_on_voice(
        tts_path="/tmp/a.mp3", media_files=[], images=[], local_files=[]
    ) is False


def test_suppress_on_with_tts_path_and_no_other_media():
    ad = _adapter(True)
    assert ad._should_suppress_text_on_voice(
        tts_path="/tmp/a.mp3", media_files=[], images=[], local_files=[]
    ) is True
...
def test_suppress_on_with_voice_media_file():
    ad = _adapter(True)
    assert ad._should_suppress_text_on_voice(
        tts_path=None, media_files=[("/tmp/voice.ogg", True)], images=[], local_files=[]
    ) is True
...
def test_suppress_on_with_image_preserves_text():
    ad = _adapter(True)
    assert ad._should_suppress_text_on_voice(
        tts_path="/tmp/a.mp3", media_files=[], images=["photo.png"], local_files=[]
    ) is False
...
def test_suppress_on_with_local_file_preserves_text():
    ad = _adapter(True)
    assert ad._should_suppress_text_on_voice(
        tts_path="/tmp/a.mp3", media_files=[], images=[], local_files=["/tmp/doc.pdf"]
    ) is False


def test_suppress_on_but_no_voice_delivered():
    ad = _adapter(True)
    assert ad._should_suppress_text_on_voice(
        tts_path=None, media_files=[], images=[], local_files=[]
    ) is False
